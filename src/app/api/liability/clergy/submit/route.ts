...
export async function POST(request: NextRequest) {
  try {
    ...
    if (access_code.startsWith('STF-')) {
      ...
      const staffLiabilityForm = await prisma.liabilityForm.create({
        data: {
      ...
      await prisma.staffRegistration.update({
        ...
        data: { liabilityFormId: staffLiabilityForm.id },
      })

      const pdfUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/liability/forms/${staffLiabilityForm.id}/pdf`
      await prisma.liabilityForm.update({ where: { id: staffLiabilityForm.id }, data: { pdfUrl } })
      ...
      try {
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="padding: 30px 20px;">
                ...
                <p>Thank you, ${greeting} ${last_name}, for completing your liability form for <strong>${staffRegistration.event.name}</strong>.</p>
                <div style="text-align: center; margin: 20px 0;">
                  <a href="${pdfUrl}" style="display: inline-block; background-color: #10B981; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold;">Download PDF Copy</a>
                </div>
        ...
        console.error('[Clergy Submit] Failed to send staff confirmation email:', emailErr)
      }

      return NextResponse.json({ success: true, form_id: staffLiabilityForm.id, pdf_url: pdfUrl })
    }
    ...
    try {
      ...
      console.error('[Clergy Submit] Failed to send confirmation email:', emailErr)
    }
    ...
    try {
      ...
      console.error('[Clergy Submit] Failed to send group leader notification:', emailErr)
    }
