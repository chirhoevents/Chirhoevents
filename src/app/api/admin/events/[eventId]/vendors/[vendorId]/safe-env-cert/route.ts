...
export async function POST(
  ...
  const { error, user, effectiveOrgId } = await verifyEventAccess(request, eventId, {
    requireAdmin: true,
    logPrefix: '[Admin Upload Safe Env Cert - Vendor]',
  })
  ...
  const url = await uploadSafeEnvCert(buffer, file.name, 'vendor', vendor.id, effectiveOrgId)
  ...
  try {
    const emailContent = wrapEmail(
      `
        ...
        <p>
          The team at <strong>${vendor.event.organization.name}</strong> received the
          Safe Environment certificate for <strong>${vendor.businessName}</strong> at
          <strong>${vendor.event.name}</strong> and uploaded it to your record on your behalf.
        </p>
        ${emailInfoBox(
      ...
      `,
      {
        ...
        preheader: `Safe Environment certificate uploaded for ${vendor.event.name}`,
    ...
    await resend.emails.send({
      ...
      to: vendor.email,
      subject: `Safe Environment Certificate Uploaded - ${vendor.event.name}`,
      html: emailContent,
    })
  } catch (emailErr) {
    console.error('[Admin Upload Safe Env Cert - Vendor] Failed to send confirmation email:', emailErr)
  }
