...
export async function DELETE(
  ...
  try {
    ...
    const { error, user, event } = await verifyEventAccess(request, eventId, {
      requireAdmin: true,
      logPrefix: '[DELETE /api/admin/events/[eventId]/poros/small-group-assignments/[id]]',
    })
    if (error) return error
    if (!hasPermission(user!.role, 'poros.access')) {
      console.error(`[DELETE Small Group Assignment] ❌ User ${user!.email} (role: ${user!.role}) lacks poros.access permission`)
