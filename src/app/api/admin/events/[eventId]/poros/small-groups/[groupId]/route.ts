...
export async function PUT(
  ...
  try {
    ...
    if (!hasPermission(user!.role, 'poros.access')) {
      console.error(`[PUT Small Group] ❌ User ${user!.email} (role: ${user!.role}) lacks poros.access permission`)
...
export async function DELETE(
  ...
  try {
    ...
    if (error) return error
    if (!hasPermission(user!.role, 'poros.access')) {
      console.error(`[DELETE Small Group] ❌ User ${user!.email} (role: ${user!.role}) lacks poros.access permission`)
      return NextResponse.json(
        { message: 'Forbidden - Poros portal access required' },
        { status: 403 }
      )
