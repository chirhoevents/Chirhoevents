...
export async function POST(
  ...
  try {
    ...
    // Expected columns: Color Name, Color Hex, For Groups, Sat Breakfast, Sat Lunch, Sat Dinner, Sun Breakfast, Capacity, Active
    const nameIndex = headers.findIndex(h => h.includes('color') && h.includes('name'))
    const hexIndex = headers.findIndex(h => h.includes('hex'))
    const accommodationIndex = headers.findIndex(h => h.includes('for') && h.includes('groups'))
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Parse accommodation type, default to 'all'
      let accommodationType = 'all'
      if (accommodationIndex !== -1) {
        const rawAccommodation = row[accommodationIndex]?.toLowerCase() || 'all'
        if (rawAccommodation === 'on_campus' || rawAccommodation === 'on-campus' || rawAccommodation === 'oncampus') {
          accommodationType = 'on_campus'
        } else if (rawAccommodation === 'off_campus' || rawAccommodation === 'off-campus' || rawAccommodation === 'offcampus') {
