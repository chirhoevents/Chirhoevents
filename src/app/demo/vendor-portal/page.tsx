...
// DEMO: hardcoded valid vendor codes
const VENDOR_CODES: Record<string, { businessName: string; eventName: string; eventDates: string; status: 'approved' | 'pending' | 'rejected' }> = {
  'VNDACC-DEMO0001': { businessName: 'Sacred Heart Rosary Co.', eventName: 'Summer Youth Retreat 2026', eventDates: 'July 15-18, 2026', status: 'approved' },
  'VNDACC-DEMO0002': { businessName: 'Word on Fire Books', eventName: 'Summer Youth Retreat 2026', eventDates: 'July 15-18, 2026', status: 'approved' },
  'VNDACC-DEMO0003': { businessName: 'Steubenville T-Shirts', eventName: 'Summer Youth Retreat 2026', eventDates: 'July 15-18, 2026', status: 'pending' },
}
...
export default function VendorPortalLanding() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const info = VENDOR_CODES[accessCode.trim().toUpperCase()]
      if (!info) {
        setError('Invalid or expired access code. Try VNDACC-DEMO0001, DEMO0002, or DEMO0003.')
        setLoading(false)
        return
      }
      setVendorInfo({ ...info, accessCode: accessCode.trim().toUpperCase() })
      setLoading(false)
    }, 500)
  }
  ...
  const handleConfirm = () => {
    if (vendorInfo) {
      router.push(`/demo/vendor-dashboard?code=${vendorInfo.accessCode}`)
    }
  }
  ...
  return (
    <div className="min-h-[calc(100vh-36px)] bg-gradient-to-b from-[#1E3A5F] to-[#1E3A5F]/90">
          ...
          {!vendorInfo ? (
            <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-12">
              ...
              <div className="mt-8 space-y-3">
                <div className="p-3 bg-[#F5F1E8] border border-[#E1D5BA] rounded text-xs text-slate-700">
                  <p className="font-medium mb-1 text-navy">Demo codes that work</p>
                  <ul className="list-disc list-inside space-y-0.5">
                    <li><code className="font-mono">VNDACC-DEMO0001</code> — Sacred Heart Rosary Co. (approved)</li>
                    <li><code className="font-mono">VNDACC-DEMO0002</code> — Word on Fire Books (approved)</li>
                    <li><code className="font-mono">VNDACC-DEMO0003</code> — Steubenville T-Shirts (pending)</li>
                  </ul>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-12">
              ...
              <div className="bg-gray-50 rounded-lg p-6 mb-8 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-semibold capitalize ${vendorInfo.status === 'approved' ? 'text-green-600' : vendorInfo.status === 'rejected' ? 'text-red-600' : 'text-amber-600'}`}>
                    {vendorInfo.status}
                  </span>
                </div>
              </div>
      ...
      <div className="border-t border-white/20 py-8 mt-12">
        <div className="container mx-auto px-4 text-center">
          <p className="text-white/80 text-sm">Powered by ChiRho Events</p>
        </div>
      </div>
    </div>
  )
